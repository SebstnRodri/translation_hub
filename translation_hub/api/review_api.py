...
def get_reviews(source_app=None, language=None, status="Pending"):
	...
	reviews = frappe.get_all("Translation Review", fields=["*"], filters=filters, order_by="creation asc")
	return reviews
...
def create_translation_learning(review, correction):
	...
	if not frappe.db.exists(
		"Translation Learning", {"review_ref": review.name, "learning_type": "Full Correction"}
	):
		doc = frappe.get_doc(
			{
		...
		doc = frappe.get_doc(
			{
				...
				or f"Term '{problematic_term}' should be translated as '{correct_term}'",
			}
		)
		doc.insert(ignore_permissions=True)
		frappe.msgprint(
