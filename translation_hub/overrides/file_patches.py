...
def patch_sidebar_item_html() -> str:
	...
	if not target_file.exists():
		print("  [sidebar_item.html] File not found, skipping")
		return "skipped"
	...
	if PATCH_MARKER in content:
		print("  [sidebar_item.html] Already patched, skipping")
		return "skipped"
	...
	content = re.sub(r'title="\{\{\s*item\.label\s*\}\}"', 'title="{{ __(item.label) }}"', content)
	...
	if content != original_content:
		...
		print("  [sidebar_item.html] ✓ Patched (added __() for translation)")
		return "applied"
	else:
		print("  [sidebar_item.html] No changes needed")
		return "skipped"
...
def revert_sidebar_item_html() -> str:
	...
	if backup_file.exists():
		...
		backup_file.unlink()
		print("  [sidebar_item.html] ✓ Reverted from backup")
		return "reverted"
	else:
		...
		if target_file.exists():
			...
			if PATCH_MARKER in content:
				...
				target_file.write_text(content)
				print("  [sidebar_item.html] ✓ Reverted (removed __() wrappers)")
				return "reverted"

		print("  [sidebar_item.html] No backup found, skipping")
		return "skipped"
...
def _trigger_asset_rebuild():
	...
	try:
		...
		result = subprocess.run(
			["bench", "build", "--app", "frappe"], cwd=bench_path, capture_output=True, text=True, timeout=300
		)
		...
		if result.returncode == 0:
			...
			if result.stderr:
				frappe.log_error(
					f"Asset rebuild warning:\n{result.stderr}", "Translation Hub - Asset Rebuild"
				)
