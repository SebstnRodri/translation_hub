...
class TestGitSyncService(FrappeTestCase):
	def setUp(self):
		...
		subprocess.run(["git", "clone", str(self.test_repo_dir), str(temp_clone)], check=True, capture_output=True)
		subprocess.run(["git", "config", "user.email", "test@test.com"], cwd=temp_clone, check=True, capture_output=True)
		subprocess.run(["git", "config", "user.name", "Test User"], cwd=temp_clone, check=True, capture_output=True)
	...
	def test_backup_creates_directory_structure(self):
		...
		try:
			...
			app_locale_dir = self.backup_repo_dir / "translation_hub" / "locale"
	...
	def test_multiple_apps_backup(self):
		...
		try:
			...
			for app_name in ["translation_hub", "frappe"]:
				app_dir = self.backup_repo_dir / app_name / "locale"
				self.assertTrue(app_dir.exists(), f"{app_name} directory should exist")
