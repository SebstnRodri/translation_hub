...
from translation_hub.translation_hub.doctype.translation_review.translation_review import create_translation_review
    ...
    def create_test_profile(self):
        if not frappe.db.exists("Localization Profile", "Test Profile"):
            doc = frappe.get_doc({
                ...
                "context_rules": [
                    {"rule_type": "Regex Pattern", "source_pattern": "Return Against (?P<doc>.*)", "target_translation": "Devolução de {doc}", "priority": 100}
                ]
            })
