...
translation_hub.ReviewCenter = class ReviewCenter {
                    ...
                    <div class="detail-content hidden">
                        ...
                        <div class="detail-body">
                            ...
                            <div class="ai-diff hidden">
                                <small class="text-warning"><strong>${__(
									"AI Original:"
								)}</strong></small>
				...
				if (r.message) {
					if (r.message.status === "Pending") {
						...
						frappe.msgprint({
							...
							message: __(
								'This review ({0}) has already been {1}. <a href="/desk/Form/Translation Review/{2}">View it here</a>.',
		...
		this.reviews.forEach((review, index) => {
			...
			html += `
                <div class="review-item" data-index="${index}" data-name="${review.name}">
                    <div class="item-source">${frappe.utils.escape_html(source_preview)}${
				source_preview.length >= 60 ? "..." : ""
			}</div>
                    <div class="item-meta">
	...
	show_detail(review) {
		...
		this.wrapper
			.find(".current-text-display")
			.text(review.translated_text || __("(no current translation)"));
		this.wrapper.find(".translation-input").val(review.suggested_text || "");

		// AI diff
		if (
			review.ai_suggestion_snapshot &&
			review.ai_suggestion_snapshot !== review.suggested_text
		) {
		...
		frappe.prompt(
			[
				...
				},
				{
					...
					description: __(
						"If a specific term is consistently mistranslated, fill in below to teach the AI."
					),
				},
				{
