...
frappe.ui.form.on("Translator Settings", {
	refresh(frm) {
		...
		frm.add_custom_button(__("Backup Translations"), function() {
			frappe.confirm(
		...
		frm.add_custom_button(__("Restore Translations"), function() {
			frappe.confirm(
				__("Are you sure you want to restore translations from the remote repository? This will overwrite local files."),
				function() {
					frappe.call({
