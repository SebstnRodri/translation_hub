...
def save_translation(task_name: str) -> dict:
	...
	existing = frappe.db.exists(
		"Translation", {"source_text": task.source_text, "language": task.target_language}
	)
...
Respond with ONLY the corrected translation, nothing else."""
	...
	try:
		...
		quality_result = evaluate_translation_quality(
			task.source_text, new_translation, settings.quality_threshold or 0.8
		)
		...
		return {"success": False, "message": f"Failed to generate new translation: {e!s}"}
...
def evaluate_translation_quality(source: str, translation: str, threshold: float) -> dict:
	...
	return {"passed": min_score >= threshold, "score": min_score, "issues": issues}
