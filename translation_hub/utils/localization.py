...
def get_localization_profile(language, app=None):
	...
	if app:
		profile = frappe.db.get_value(
			...
			{"language": language, "app": app, "is_active": 1}, 
			"name"
		)
		if profile:
			return profile
	...
	# Fallback to generic
	filters = {"language": language, "is_active": 1}
	...
	return frappe.db.sql("""
		...
		LIMIT 1
	""", (language,), as_dict=True)[0].get("name") if frappe.db.sql("""
		SELECT name FROM `tabLocalization Profile`
		WHERE language = %s AND is_active = 1
		AND (app IS NULL OR app = '')
		LIMIT 1
	""", (language,), as_dict=True) else None
